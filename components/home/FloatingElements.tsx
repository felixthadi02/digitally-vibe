...
export default function Footer() {
  ...
  useEffect(() => {
    // Set mounted state and year on client side only
    setIsMounted(true);
    setCurrentYear(new Date().getFullYear());
  }, []);

  useEffect(() => {
    // Only run GSAP animations after component is mounted (client-side)
    if (!isMounted) return;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 pb-16">
          ...
          <div className="lg:col-span-1">
            ...
            {/* Social Links - Removed inline styles that GSAP adds */}
            <div className="flex gap-3">
              {social.map((item, index) => (
                <a
                  key={index}
                  href={item.href}
                  className={`${isMounted ? 'float-icon' : ''} w-10 h-10 bg-gray-800 hover:bg-[#ff3233] rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all duration-300`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <p className="text-gray-500 text-sm">
              © {currentYear ?? '2024'} DigitallyVibe. All rights reserved.
            </p>
